...
//初始化
function init() {
    var len = $('#ms-img img').length; //获取图片有多少张
    var html = '';
    //拼html
    for (var i = 0; i < len; i++) {
    ...
    $('#num').html(html);
    //显示第一张图片
    $('#ms-img img').first().css('display', 'block');
    //第一个数字背景颜色
...
function changImg(num) {
    ...
    //链式写法
    $('#ms-img img').eq(num).css('display', 'block').siblings().css('display',
        'none');
    $('#num li').eq(num).addClass('active').siblings()
        .removeClass('active');
    index++;
    if (index == $('#ms-img img').length) { //最后一张
        index = 0; //第一张
    }
}
var index = 0;
